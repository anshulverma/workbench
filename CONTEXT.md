...
**Triage Queue**: Durable, priority-ordered list of triage cards waiting to be sent to Google Chat, persisted as columns on the `triage_cards` table in PostgreSQL (not a separate table). Cards are ordered by relevance score (highest first) and sent one at a time, sequentially. Queue status lifecycle: `queued → sent → responded / expired`. Advances on response or timeout. Has a configurable daily cap (default 20). Cards older than a configurable expiry (default 7 days) are auto-included at P3. The bot confirms each response before advancing to the next card. Each card stores its `bot_message_id` for response polling recovery across restarts.
...
**Source Adapter**: A provider that polls an external system (GitHub, Phabricator, email, etc.) and produces raw items. Fetch only — no semantic extraction. Loaded at runtime via dynamic import from the YAML config file.
...
- Cards are sent to Google Chat one at a time; the bot confirms each response
...
- **Prometheus** scrapes metrics from the server; **Grafana** provides dashboards and alerts to Google Chat
...
> → Check queue health in Grafana. If the LLM provider is rate-limited, the worker can't dequeue fast enough. Consider bumping `queue.worker_concurrency` in config.yml, or check for dead-letter items that may indicate a persistent failure.
...
- **Single Messenger** — replaced by pluggable messenger system. Discord is the default external messenger; Google Chat is the internal (Meta) messenger. Phase 1a interface has `send_triage_card`, `send_notification`, `poll_responses`, `close`. Rich interface (buttons, reactions, threads) deferred to the internal/external split.
...
- **`subprocess.run` for external calls** — replaced by `asyncio.create_subprocess_exec` in all providers (GoogleChatMessenger, PhabricatorAdapter, etc.) to avoid blocking the event loop.
