...
## Observability & call-reduction (plugboard)

**Plugboard**: the Meta-internal LLM gateway (`https://plugboard.x2p.facebook.net`) that the `AnthropicLLM` provider, the `LLMQueueScorer`, and the memory service's Graphiti client all call. The call-reduction + observability work targets it. _Avoid_: "the LLM" alone (ambiguous across the three clients).

**Plugboard client**: which of the three callers issued a plugboard request, carried as the `client` metric label: `main_llm` (AnthropicLLM/MetaAnthropicLLM), `queue_scorer` (LLMQueueScorer/MetaQueueScorer), `memory` (Graphiti, separate process). _Avoid_: "provider" (overloaded with the YAML provider system).

**Call vs operation vs item**: a **call** is one `messages.create` HTTP attempt to plugboard (includes retries) — counted by `plugboard_calls_total`. An **operation/method** is one logical `LLMProvider` method invocation (extract, score_relevance) — counted by the existing `llm_calls{method}` decorator view. An **item** is one logical unit folded into a (possibly batched) call — counted by `plugboard_items_total`. These three intentionally differ (retries inflate calls; batching folds many items into one call). _Avoid_: using "call" loosely.
