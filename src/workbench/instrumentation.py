...
class InstrumentedSourceAdapter:
    ...
    async def poll(self, since: datetime | None = None):
        ...
        try:
            ...
            self._metrics.adapter_polls.labels(
                adapter=self._name, status="success"
            ).inc()
            self._metrics.adapter_poll_seconds.labels(adapter=self._name).observe(
                elapsed
            )
            for item in items:
...
class InstrumentedLLMProvider:
    ...
    async def score_relevance_many(self, *args, **kwargs):
        return await self._instrumented_call(
            "score_relevance_many", self._inner.score_relevance_many(*args, **kwargs)
        )
...
class InstrumentedContextEnricher:
    ...
    async def enrich(self, *args, **kwargs):
        ...
        try:
            ...
            elapsed = time.monotonic() - start
            self._metrics.enrichment_seconds.labels(enricher=self._name).observe(
                elapsed
            )
            logger.info(
                "enrichment_complete",
