...
**Action Item (User Todo)**: An `Item` with `action_source` set, created from a free-text triage response. Links to the triggering item via `parent_item_id`. Has two category fields that serve different purposes: `category` = `ACTION_ITEM` (what kind of content — set during LLM extraction, same enum as all Items) and `action_category` = one of `ActionCategory` enum values (what kind of action the user needs to take — delegation, communication, scheduling, review, creation, update — set by interpret_triage_response). In v1, all action items are surfaced as user todos. In v2, an Action Module may execute some automatically.
