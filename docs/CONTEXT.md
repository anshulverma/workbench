...
**Interpreted Response**: The LLM's structured interpretation of a free-text triage reply. Contains `system_actions` (executable now: add_todo, skip, mute_pattern, defer) and `user_todos` (create as action items for the user). Destructive system actions (skip, mute) require user confirmation before execution. The interpreter receives the full context stored on the card at generation time (enrichment context, entity knowledge, relationships, preference facts). All interpreted responses are logged for future model training.
_Avoid_: "parsed response" (it's LLM interpretation, not parsing)

**Adapter State**: Persistent state for source adapters that need to track information across restarts (e.g., GChat's tracked_threads map). Stored in the `adapter_state` PG table as JSONB keyed by adapter name. Accessed via `AdapterStateStore` interface, injected into adapters via `state_store` constructor kwarg (same `inspect.signature` pattern as connection injection).
