...
- `router = APIRouter(prefix="/api/client-logs", tags=["client-logs"])`, added to
  the router list in `runtime/app.py` (the `include_router` loop, ~line 542-566).
- Pydantic models:
  - `ClientLogEvent`:
...
(`telemetry/privacy.py`, wired into the structlog chain in
`telemetry/logging.py:128`) already redacts emails/phones and truncates long
strings on every event. One change: add `"stack"` and `"component_stack"` to
`NO_TRUNCATE_KEYS` so traces are kept full (still PII-redacted) rather than cut
at `max_content_in_logs`.
...
- **Server (pytest):**
  ...
  - valid batch → `204`; structlog emits one record per event at the mapped
    level with the expected structured fields.
  - oversized batch (> 50 events) → `413`.
  - field truncation/redaction applied; `stack` survives full (not truncated).
- **Client (vitest):**
...
- `src/workbench/telemetry/privacy.py` (`NO_TRUNCATE_KEYS` += stack keys)
