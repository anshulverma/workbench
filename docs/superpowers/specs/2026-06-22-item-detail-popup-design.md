...
1. **Adapter + type** — `lib/types/search.ts` stays the UI contract; add
   ...
   `source_type→source`, `status→state`, `enriched_context→context`,
   `processing_log→stages`, `relevance` default `0`, `verdict` mapped (the API
...
New:
- `src/workbench/api/items.py` (add `by-id` route + shared mapper — modify)
