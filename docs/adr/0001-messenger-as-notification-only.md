...
The bot uses **text-based replies** (user types "1", "2", "3") as the primary input method. Triage cards are sent **one at a time, sequentially** to eliminate response-matching ambiguity. The API, the web UI Triage page, and the `make triage` CLI (`scripts/triage.py`) remain available as alternative input methods.
