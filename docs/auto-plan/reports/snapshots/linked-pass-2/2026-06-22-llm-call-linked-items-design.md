...
```
LLM detail dialog ──GET /api/llm/calls/{id}──► get_call_detail
                                                 ├─ linked_items =
...
- **`EntityLinkStore.linked_items_for_correlation(correlation_id: str | None) -> list[LinkedItem]`**
  ...
  shape `{id: int, path: str, summary: str}` (`item_path` is `TEXT NOT NULL`, so
  `path` is never null; the plan implements it as a frozen dataclass). entity_links is
...
- The `LLMCallDetail` dialog renders a **"Linked items"** section as a top-level
  sibling immediately after the header block and before the system-prompt block
  (NOT nested in the `call.batch > 1` subcall-selector block — non-batched calls
  have links too): a wrapped row of chips, each
  `<ItemLink id={it.id}>#{it.id} · {it.summary}</ItemLink>` (truncated summary).
  `ItemLink` (from the prior feature) is a `<button>` that opens the `?item=<id>`
  item-detail dialog via `useItemDialog`/`useSearchParams`; the app-root
