...
`privacy` differs. **`AppConfig` enforces required-no-default fields**: `storage`
(always supplied by the Tier-1 DSN) and `llm` (`config/models.py:162`, `dict`, no
default, no stub provider). With neither a seed nor an `llm` override present,
construction **hard-fails** with a clear message (seed via `WORKBENCH_CONFIG_SEED`
or `PATCH /api/settings/llm` after first boot) — symmetric to the missing-DSN
hard-fail. **Convert the uvicorn-binding entrypoints**: `__main__.py:main`
...
- `src/workbench/config/assembly.py` — `assemble_config(stores, bootstrap)` (merge + resolve → `AppConfig`; hard-fails on missing `llm`)
...
**Tests:**
- `tests/test_config_assembly.py` — defaults ⨉ document ⨉ sources ⨉ env-resolve (document supplies `llm`); real env wins over `.env`; unset referenced secret fails fast; missing DSN hard-fails; **missing `llm` (no seed, no `llm` override) hard-fails with a clear message**
...
- `tests/test_config_importer.py` — seed → document + `source_configs`; idempotent (2nd boot no-op); no-seed → defaults for every defaultable section **while still requiring `llm`** (assert hard-fail when `llm` absent); secrets stored unresolved (not baked); `change_detector`/`relevance` mapped
