...
def test_ingest_redacts_pii_and_keeps_stack_untruncated():
    ...
    try:
        _client().post(
            ...
            json={
                "events": [
                    _event(message="contact me@example.com", stack=long_stack)
                ]
            },
        )
    finally:
...
> `PrivacyConfig` lives at `src/workbench/config/models.py:99` and all fields
> have defaults (`sanitize_logs`, `redact_emails`, `redact_phones`,
> `max_content_in_logs`, `allow_pii_in_debug_logs`), so the kwargs above are
> safe. The redaction replacement strings (`[REDACTED:email]`) are defined in
> `SanitizingProcessor` (`telemetry/privacy.py`).
...
In `src/workbench/runtime/app.py`, the route imports are a single alphabetized `from workbench.api import ( ... )` block inside `create_app()` (~lines 513-539) and the registration is the `for r in [ ... ]: app.include_router(r)` loop (~lines 541-568, last entry `llm.router`). Add `client_logs,` to the import tuple (alphabetically, between `auth_token` and `config as config_api`) and add `client_logs.router,` as the last entry of the `for r in [ ... ]` list (after `llm.router,`).
...
Add to `ui/src/lib/api.test.ts` (a new test). **Important — match the existing
file's harness:** `api.test.ts` uses **MSW** (`setupServer` / `server.use(...)`,
`http`, `HttpResponse` from `msw`) to intercept requests — do NOT
`vi.stubGlobal('fetch', ...)` (that replaces fetch and defeats MSW's
interception). Also note `vi` is **not** currently imported in this file (the
import line is `import { describe, it, expect, beforeAll, afterAll, afterEach }
from 'vitest'`) — add `vi` to that import. The file's `afterEach` already calls
`_resetToken()`, so the module-cached token is cleared between tests; the default
`/api/auth/token` handler returns `{ token: 'tok-123' }`, so `getToken()`
succeeds before the data request fails.
...
Add the reporter import near the top:

```ts
import * as reporter from './error-reporter'
```

Add the test (inside the existing `describe('api client', ...)` block):
