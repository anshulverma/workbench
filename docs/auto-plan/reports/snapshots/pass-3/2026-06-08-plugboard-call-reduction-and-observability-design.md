...
- **Relevance routing:** `score_and_decide` (in `filter.py`) currently does per-item memory queries (preferences/entity/relationships) + filter-rule fetch + one `score_relevance`, then applies the threshold branch inline (`include_threshold=70`/`drop_threshold=30`/`confidence_threshold=70`, from `PipelineConfig`). Extract that threshold branch into a **pure helper `decide_from_score(relevance, confidence, *, include_threshold, drop_threshold, confidence_threshold) -> str` that lives in `filter.py`** (the module that owns the thresholds today); `score_and_decide` calls it after `score_relevance`, and the engine calls it directly when handed a precomputed score (no duplicated threshold constants). The batch refactor gathers per-item facts (via `asyncio.gather`), calls `score_relevance_many` once, then routes each item through the retained single-item helper `_process_extracted_item` (given the precomputed `(relevance, confidence)`, which calls `filter.decide_from_score` instead of `score_and_decide`) so per-item auto_include/auto_drop/triage routing, per-item job counters, item commit, and the §3.1 drop-recording gate are all preserved.
