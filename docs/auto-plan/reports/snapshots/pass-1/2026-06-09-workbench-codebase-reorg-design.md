...
- **Packaging**: `pyproject.toml` uses `packages.find where=["src"]` — auto-discovers new packages that contain `__init__.py`; no manual edit, but **every new package dir must have `__init__.py`**.
...
1. `grep -rEn '"workbench\.[a-z_.]+\.[A-Z][A-Za-z]*"' src/ config*.yml` → every hit resolves to an existing module.
2. `ruff check src/ tests/` and `python -c "import workbench.runtime.app"` → static-import integrity.
