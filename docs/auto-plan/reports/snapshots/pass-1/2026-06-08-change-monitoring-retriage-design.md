...
| DebounceManager | `pipeline/scheduler.py` | 2-minute trailing-edge timer per `item_id`, merges rapid changes |
...
async def _route_poll_results(
    ...
    for raw_item in raw_items:
        ...
        await self.stores.items.update_raw_data(existing.id, raw_item.raw_text)

        if result.is_material:
            self._debounce_mgr.schedule(existing.id, result, raw_item)

    # Detect disappeared items (in DB but absent from poll results)
    if adapter.supports_monitoring():
...
class DebounceManager:
    ...
    def schedule(self, item_id: str, result: ChangeResult, raw_item: RawItem) -> None:
        ...
        if item_id in self._pending:
            old = self._pending[item_id]
            if old.timer_handle is not None:
                old.timer_handle.cancel()
            merged = self._merge_results(old.merged_result, result)
        else:
            merged = result
    ...
    async def _fire(self, item_id: str) -> None:
        ...
        if entry is None:
            return
        await self._fire_callback(entry.item_id, entry.merged_result, entry.raw_item)
...
async def _fire_retriage(
    ...
    old_raw = _parse_raw(item.raw_data)
    new_raw = json.loads(raw_item.raw_text)
...
class ItemStore(ABC):
    ...
    async def update_raw_data(self, item_id: str, raw_text: str) -> None:
        """Update Item.raw_data with fresh source payload."""
        ...
```
...
| `src/workbench/providers/source/base.py` | Add `supports_monitoring()` and `stable_id()` concrete methods |
