...
Add to `tests/test_llm_writer.py` (import `_to_llm_record` and `PlugboardCallRecord`; reuse existing context helpers in that file if present — otherwise build a context inline as shown):
...
def test_to_llm_record_threads_raw_io():
    ...
    rec = _to_llm_record(
        PlugboardCallRecord(
            client="main_llm", model="m", context=_ctx(),
...
- Modify: `src/workbench/providers/queue_scorer/llm.py` (~line 182)
- Test: `tests/test_anthropic_llm.py`

> Each call site builds **one** `request` dict and uses it for both `messages.create(**request)` and `raw_request=request`, so the stored request always equals what was sent. `_call_with_retry` is the chokepoint for single calls **and** batch relevance scoring, so wiring it covers both.
...
```python
async def test_detail_view_exposes_raw_io(stores, app_client):
    from datetime import datetime, timezone
    ...
    resp = await app_client.get(f"/api/llm/calls/llm_{row.id}")
...
> If `tests/test_llm_api.py` builds its client differently (e.g. a `client` fixture or an inline `TestClient`/`httpx.AsyncClient`), match that file's existing pattern rather than the `stores, app_client` signature shown — read the top of the file first.
...
function RawSection({
  ...
  return (
    <div>
      ...
      >
        {getIcon(open ? 'ChevronDown' : 'ChevronRight')}
        {label}
      </button>
      {open && (
        <div
...
> Confirm `useState` and `getIcon` are already imported in this file (they are used by the existing dialog); if `getIcon` is not imported here, import it from the same module `EnricherCard.tsx` uses (`grep "getIcon" ui/src/components/funnel/EnricherCard.tsx` to find the import path).

5b. Render the two sections at the **end of the body div**, after the per-subcall block closes (after line 1177's closing `</div>`, still inside the scrollable body container so they belong to the call):
