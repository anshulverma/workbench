...
- **`llm` is a required Tier-2 value, not a defaultable one.** `AppConfig.llm`
  is `dict` with **no pydantic default** (`config/models.py:162`) and there is no
  stub/console LLM provider (`providers/llm/` ships only `anthropic` + the
  `plugboard` wrapper). So a valid `AppConfig` cannot be assembled unless `llm`
  is present in the settings document or the seed. On a no-seed first boot,
  assembly **hard-fails** with a clear message ("set the `llm` section via
  `WORKBENCH_CONFIG_SEED` or after first boot via `PATCH /api/settings/llm`"),
  symmetric to the missing-DSN hard-fail. (`storage.postgres_dsn`, the only other
  required-no-default field, is always supplied by Tier-1.)
...
5. Assemble: `pydantic defaults` ⨉ `deep-merge(settings document)` ⨉ `sources`
   ...
   `AppConfig(**resolved)` enforces required-no-default fields: `storage` is
   always present (Tier-1 DSN); **`llm` must be present in the document/seed or
   construction hard-fails** (see the `llm` edge case above).
...
4. If no seed exists, leave the row absent and run on pydantic defaults — which
   still requires an `llm` section to exist; with neither a seed nor an `llm`
   override, assembly hard-fails (see the `llm` edge case). A DSN must also have
   reached the DB at all.
...
- **Missing `llm` at startup:** `llm` is required-no-default with no stub
  provider. With neither a seed nor an `llm` override in the document, assembly
  hard-fails with a clear message naming the fix (seed via `WORKBENCH_CONFIG_SEED`
  or `PATCH /api/settings/llm` after first boot). All other sections fall back to
  pydantic defaults.
- **First boot, no seed (DSN + `llm` present):** every defaultable section uses
  pydantic defaults — `sources: []`, console messenger, etc.
...
- **Backend unit:** `SettingsStore` round-trips + single-row invariant; assembly
  ...
  fails fast; missing-`llm` hard-fails; `extra="ignore"` drops stale keys);
  one-time import (seed → document + `source_configs`, idempotent, no-seed →
  defaults with required-`llm` still enforced, secrets not baked);
