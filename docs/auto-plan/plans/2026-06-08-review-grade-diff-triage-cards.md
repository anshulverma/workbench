...
- ADRs 0022–0029 already exist in `docs/adr/` and cover every decision in this spec; **do not create a new ADR file**. Reference them in commit messages.
    ...
    def render(self, card: TriageCard, item: ExtractedItem) -> CardMessage:
        ...
        try:
            ...
            logger.warning(
                ...
                source_type, type(delegate).__name__, item.raw_item.id, exc,
            )
            return self._default.render(card, item)
...
NOTE: `ExtractedItem` has no `.source_type`/`.id`; the composite reads `item.raw_item.source_type` and `item.raw_item.id`. `ValidationError` is imported at the top of `presenter.py` (`from pydantic import ValidationError`). The three fallback log events match the spec failure table (d6): `presenter_fallback` (no delegate registered for `source_type`), `presenter_content_invalid` (delegate raised a pydantic `ValidationError` while validating `card_content['sections']`), and `presenter_failure` (any other delegate exception) — all route to `PlainCardPresenter`. Adjust the test helper so `comp.render` keys off `item.raw_item.source_type`, and add `test_composite_content_invalid_logs_and_falls_back` (delegate whose `render` raises `ValidationError`; assert plain output + `presenter_content_invalid` via `caplog`).
