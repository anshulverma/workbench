...
All three are concrete with backward-compatible defaults. Not abstract.
...
1. Minimal subclass inherits `supports_monitoring() == False`, `stable_id()` returns `raw_item.id`, `poll_returns_complete_set()` returns `False`.
2. Monitoring-enabled subclass overrides all three.
...
3. `update_raw_data` with a `RawItem` replaces the JSONB with `raw_item.model_dump()` and bumps `updated_at`.
...
The callback is passed at construction (not per-schedule call). `schedule()` accepts a `ChangeResult`, `RawItem`, and `old_raw` dict (snapshot of raw data before `update_raw_data` overwrites it). The first detection in a debounce window preserves the original `old_raw`; subsequent merges keep the earliest `old_raw`. Consecutive calls for the same `item_id` merge `ChangeResult` fields (union of `changed_fields`, OR of `is_critical`). The callback receives `(item_id, merged_result, raw_item, old_raw)`.
...
8. Monitoring-enabled adapter with `poll_returns_complete_set() == False`: `_detect_disappeared` NOT called, items absent from poll results remain active.
