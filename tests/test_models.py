...
def test_item_defaults():
    ...
    assert item.id is None  # DB-assigned on save
    assert item.created_at


def test_raw_item():
    raw = RawItem(
        id="D12345_123", source_type="diff", source_label="D12345", raw_text="content"
    )
...
def test_ingestion_queue_entry_defaults():
    entry = IngestionQueueEntry(raw_content="test", source_type="manual", job_id=1)
...
def test_interaction_entry_choice_index_set():
    entry = InteractionEntry(
        source_type="diff", item_summary="Review D123", choice_index=2
    )
...
def test_triage_option_suggested_fields():
    ...
    opt2 = TriageOption(
        ...
        action="skip",
        suggested=True,
        suggestion_reason="you usually skip bot PRs",
    )
...
def test_item_action_fields():
    ...
    assert item2.parent_item_id == 123
...
def test_triage_response_choice_optional():
    resp = TriageResponse(card_id=1, choice=2)
    assert resp.choice == 2
    resp2 = TriageResponse(card_id=1, raw_text="add as P3")
